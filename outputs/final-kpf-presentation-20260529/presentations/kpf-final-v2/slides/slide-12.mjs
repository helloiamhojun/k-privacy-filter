...
export async function slide12(presentation, ctx) {
  ...
  ctx.addText(slide, {
    text: "결론: 최종 구현체는 OPF의 범용 탐지력, 한국형 Privacy Rule Pack, 문맥 오탐 억제를 결합한 데모 가능한 시스템입니다. Fine-tuning은 저장까지 성공했지만, 현재는 연구 트랙으로 분리했습니다.",
    x: 80,
    y: 224,
    width: 1010,
    height: 72,
  ...
  metric(slide, ctx, 116, 384, 260, "Eval300 F1", "0.7294", "rule-pack 확장 후 replay", C.blue);
  ...
  smallText(slide, ctx, "예상 질문 답변 방향: hard-negative 0은 샘플 기준, 모델명·문서 버전은 의도적 제외, 최종 시연은 hybrid KPF입니다.", 184, 552, 900, 24, 13, C.ink);
  footer(slide, ctx, 12);
  return slide;
}
