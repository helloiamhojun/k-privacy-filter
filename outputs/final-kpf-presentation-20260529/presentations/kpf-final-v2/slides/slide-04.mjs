...
export async function slide04(presentation, ctx) {
  ...
  bg(slide, ctx);
  title(slide, ctx, "최종 시스템 아키텍처", "OPF + Korean Privacy Rule Pack + 문맥 오탐 억제", "모델 하나에 모든 책임을 맡기지 않고, 규칙과 문맥 필터를 결합한 방어적 구조를 선택했습니다.");
  const y = 238;
  ...
  flowBox(slide, ctx, 502, y, 205, "3. Rule Pack", "고유식별정보, 주소, 인증,\n금융, 네트워크 정보 보강", C.softTeal);
  ...
  smallText(slide, ctx, "scripts/pipeline.py: OPF 결과와 rule-pack 결과 병합\nscripts/regex_safety_net.py: 한국형 패턴, 문맥 false-positive suppression\nscripts/check_regex_rule_cases.py: rule-pack 회귀 테스트\nscripts/evaluate_final_kpf_offline.py: 재현 가능한 최종 평가", 148, 494, 900, 58, 12, C.muted);
  footer(slide, ctx, 4);
  return slide;
}
