...
최종 구현체는 OpenAI Privacy Filter만 쓰지 않고, 한국형 Privacy Rule Pack과 문맥 기반 오탐 억제를 결합했습니다.
최종 rule-pack 확장 후 replay 기준 Eval300 F1은 0.7294, adversarial F1은 0.9000입니다.
...
셋째, Privacy Rule Pack이 고유식별정보, 주소, 전화번호, 계좌, 인증정보, 네트워크 식별자 패턴을 보강합니다.
...
탐지 보강은 고유식별정보, 전화번호, 계좌, 카드, CVC, OTP, 주소, IP, MAC, 학번, API key 등을 담당합니다.
문맥 억제는 예시, 샘플, 더미, 상품코드, 모델명, 문서 버전, placeholder 같은 오탐을 줄입니다.
...
기존 hybrid는 0.6710이었고, 최종 KPF는 rule-pack 확장 후 0.7294까지 올라갔습니다.
Precision은 0.7994, Recall은 0.6706이고, false positive는 71개입니다.
...
"모델명은 900101-1234567-A입니다"와 "문서 버전은 sk-proj-example-not-real입니다"는 개인정보처럼 보이지만 실제 개인정보가 아닌 문맥입니다.
이 경우는 일부러 마스킹하지 않아야 합니다. 무조건 많이 지우는 것이 아니라, 개인정보가 아닌 값까지 지우는 오탐을 줄이는 것이 목표입니다.
...
## 12장. 마무리
K-Privacy Filter는 OPF의 범용 탐지력, 한국형 Privacy Rule Pack, 문맥 오탐 억제를 결합한 데모 가능한 hybrid defense입니다.
최종 수치는 Eval300 F1 0.7294, hard-negative 100개 기준 0 FP, adversarial F1 0.9000입니다.
